...
public class TakeSlow : Effect
{
    ...
    public override IEnumerator ApplyEffect(GameObject target, HitInfo info)
    {
        ...
        stats.SetEffectMultiplier(slowAmount);

        // Handle Particles (Your existing logic is fine)
        ...
        while (elapsed < slowDuration)
        {
            ...
            if (fadedSlow)
            {
                // Smoothly return the MULTIPLIER to 1.0
                float currentMult = Mathf.Lerp(slowAmount, 1f, elapsed / slowDuration);
                stats.SetEffectMultiplier(currentMult);
            }
            yield return null;
        }

        // Reset multiplier to 1.0 (Full Speed)
        stats.SetEffectMultiplier(1f);
    }
}
